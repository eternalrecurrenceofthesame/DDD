...
#### + 도메인 서비스와 이벤트를 구분하기
```
도메인 서비스는 애그리거트의 책임 범위의 문제를 해결하면서 부가적인 기능을 추가하는 것이 목적이다.
부가적인 기능이란 기본적으로 도메인 요구 사항의 성격과 비슷하면서 추가적인 요구를 의미한다. 

이벤트는 도메인에 부가적인 기능을 추가하는 것이 아닌 과거에 벌어진 일을(요구 사항) 처리하기 위해 사용된다.
성격이 다른 요구 사항을 처리하면서 트랜잭션의 문제를 해결할 수 있다. 
```
...
이벤트 생성 주체 (도메인 모델) -> 이벤트 디스패처(퍼블리셔) -> 이벤트 핸들러(구독자)
...
이벤트를 사용함으로써 애그리거트에 다른 도메인 로직(환불) 이 섞이는 것을 방지할 수 있다.
...
이벤트를 구현해서 시스템 간 강 결합 문제는 해결했지만 문제가 남아 있다. 응용 서비스에서 주문을 취소 하면 환불 로직이 호출된다.
...
'회원 가입 신청을 하면 인증 이메일을 보내라' 에서 회원 가입 신청은 이벤트가 되고 인증 이메일을 보내라 기능은 이벤트를 처리하는 핸들러에서 처리할 수 있다.

'A 하면 이어서 B 하라'는 요구 사항 중 'A 하면 최대 언제까지 B 하라' 로 바꿀 수 있는 요구 사항은 이벤트를 비동기로 처리하는 방식으로 구현할 수 있다.

```
* 이벤트를 비동기로 처리하는 네 가지 방법
...
이벤트 핸들러를 별도 스레드로 실행한다. 별도의 스레드로 실행하므로 도메인에서 이벤트를 생성하고 디스패처가 핸들러를 호출하는 것을 기다리지 않아도 된다.

핸들러가 이벤트를 처리하는 것은 별도의 스레드가 맡아서 수행한다.

```
* OrderCanceledEventHandler 참고

부트 스트랩 클래스에 @EnableAsync 애노테이션을 활성화하고 핸들러에 메서드에 @Async, @EventLitener 설정을 추가해준다.
```
### 메시지 큐를 이용한 비동기 처리 

카프카, 래빗MQ 를 이용한 비동기 처리는 이벤트를 발행하고 메시징 큐에 이벤트를 보낸다, 메시지 큐는 이벤트를 메시지 리스너에 전달하고 메시지 리스너는 알맞은

이벤트 핸들러를 이용해서 이벤트를 처리한다. 
...
또 글로벌 트랜잭션을 지원하지 않는 메시지 시스템도 있다.

마이크로서비스에서 스프링 클라우드 스트림을 이용한 이벤트 메시지 처리는 아래 내용을 참고한다.

https://github.com/eternalrecurrenceofthesame/Spring-MSA/tree/main/part01/ch07

### 이벤트 저장소를 이용한 비동기 처리

도메인에서 이벤트를 발행하고 이벤트 디스패처를 통해 직접 이벤트 핸들러에 전달하는 것이 아닌 로컬 핸들러를 통해서 로컬 저장소에 이벤트를 저장하고 API 나 포워드를 통해서 로컬 저장소에 있는 이벤트 데이터를 제공해줄 수 있다.

쉽게 말해서 로컬 데이터베이스에 이벤트를 모아뒀다가 이벤트 데이터를 제공해준다는 의미 

이벤트 저장소 방식은 API 와 포워더를 사용해서 구현할 수 있다. - eventstore 패키지 참고 

```
* 이벤트 비동기 처리에 사용되는 컴포넌트
...
EventStore - 로컬 저장소. JdbcTemplate 을 이용해서 이벤트를 저장소에 저장한다. 이벤트는 과거에 발생한 사건이기 때문에
             이벤트 데이터는 변경되지 않는다 이벤트 저장소는 이벤트를 추가하는 기능과 조회하는 기능만 제공한다.
```
```
* API 방식으로 이벤트 저장소 처리하기 

외부 클라이언트는 일정한 간격으로 EventApi 에 이벤트 데이터를 요청한다 이때 offset, limit 정보를 보내는데 offset 은
클라이언트에서 관리하는 이벤트 세이브 포인트가 되고 limit 은 요청 이벤트의 제한된 개수가 된다. 
...
클라이언트에서 이벤트 처리에 오류가 발생하더라도 실패한 이벤트부터 읽어와서 이벤트를 처리를 재시도 한다. 

순서

Order -> Events 디스패처 이벤트 출판 -> EventStoreHandler 로컬 핸들러 -> JdbcEventStore 로컬 저장소 호출
                                                        <- EventAPI <- 이벤트 핸들러에서 API 로 읽어 온 데이터 처리
```
```
* 포워더 구현

포워더는 일정 주기로 EventStore 에서 이벤트를 읽어와 이벤트 로컬 핸들러에 전달해 준다. API 와 마찬가지로 마지막에 읽은
값을 기억하고 마지막에 처리한 offset 부터 이벤트를 가져오면 된다.
...
로컬 저장소에 이벤트를 저장하는 것까지는 API 방식으로 이벤트를 처리하는 것과 똑같다. 이벤트를 읽고 이벤트를 처리하는 핸들러로
전달하는 역할은 내부 애플리케이션에서 포워더가 수행한다.


포워더는 주기적으로 이벤트 저장소에서 이벤트를 가져와서 이벤트 핸들러를 실행한다. 이벤트 처리 중 예외가 발생하더라도 다음 주기에
스케줄러를 호출할 때 재처리하게 할 수 있다.

포워더는 별도의 스레드를 이용하기 때문에 이벤트의 발행과 처리는 비동기로 처리된다.
...
반면 앞서 설명한 로컬 핸들러를 사용해서 이벤트를 비동기로 처리할 경우 이벤트 처리에 실패하면 이벤트를 유실하게 된다. 도메인이 이벤트를 생성하고

디스패처로 핸들러를 호출해서 이벤트를 저장할 때 이벤트 처리에 실패하면 이벤트 자체가 사라지게 된다는 의미.

* 이벤트 순서

이벤트 저장소를 사용하면 이벤트를 순서대로 저장하고 그 순서대로 이벤트 목록을 제공하는 반면 메시징 시스템은 사용 기술에 따라 이벤트 발생 순서와 메시지 전달 순서가 다를 수 있다.

* 이벤트 재처리에 관해서 ?? 

동일한 이벤트를 다시 처리해야 할 때 어떻게 할지를 결정해야 한다. 가장 쉬운 방법으로는 마지막으로 처리한 이벤트의 순번을 기억해 두었다가 이미 처리한 순번의 이벤트가 도착하면 해당 이벤트를 처리하지 않고 무시하는 것이다. 

예를 들어 회원 가입 이벤트가 발생하면 이메일을 발송하는데 동일한 순번의 이벤트가 다시 들어올 시 이메일을 발송하지 않는 방식으로 구현한다. 339 p 

#### + 이벤트 처리와 DB 트랜잭션 고려
```
* 이벤트를 동기로 처리할 때

외부 시스템을 호출해서 결제를 취소하고 응용 서비스에서 리턴값으로 DB 를 업데이트 할 때 실패한다면? 결제는 취소됐는데
DB 에는 주문이 취소되지 않은 상태가 된다.

* 이벤트를 비동기로 처리할 때 

이벤트 저장소에 주문 취소 이벤트를 저장하고 결제 취소 API 를 비동기로 호출했는데 실패한다면 주문은 취소가 되지만 결제는
취소되지 않은 상태로 남게 된다.
```

결론은 이벤트 처리를 **동기로하든 비동기로하든 이벤트 처리의 실패와 트랜잭션 실패를 고려해야 한다는 의미** 트랜잭션 실패와 이벤트 처리 실패를 모두 고려하면 복잡해지므로 경우의 수를 줄이면 도움이 된다.  **트랜잭션이 성공할 때만 이벤트 핸들러를 실행하는 것**
...
이 값을 지정하면 스프링은 트랜잭션 커밋에 성공한 뒤에 핸들러 메서드를 실행한다. 이 기능을 사용하면 이벤트 핸들러를 실행했는데 주문 취소 트랜잭션이 롤백 되는 상황은 발생하지 않는다.

주문 취소 트랜잭션이 커밋되고 그 뒤에 결제 취소를 호출해서 동기 이벤트로 전달한다는 의미

* 이벤트 처리를 비동기로 할 때

이벤트 발생 코드와 이벤트 저장 처리를 한 트랜잭션으로 처리하면 된다. (로컬 핸들러를 이용한 저장소 저장까지) 이렇게하면 트랜잭션이 성공할 때만 이벤트가 DB 에 저장되고 트랜잭션은 실패했는데 이벤트 핸들러가 실행되는 상황은 발생하지 않는다.
